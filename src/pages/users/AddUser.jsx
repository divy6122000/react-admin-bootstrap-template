...
import { submitUserData } from '../../features/users/userAddSlice'
...
const AddUser = () => {
    ...
    useEffect(() => {
        if (userAdd.status === 'loading') {
            document.getElementById('submit').innerHTML = 'Please Wait...';
        }
        else if (userAdd.status === 'succeeded') {
            ...
            document.getElementById('submit').innerHTML = 'Submit';
            toast.error(userAdd.error);
        }
    }, [userAdd.status, dispatch])
